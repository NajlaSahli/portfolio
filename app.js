...
(function () {
  ...
  function buildCV() {
    const head = document.querySelector('.page-header .container');
    if (head) head.innerHTML = `
      <div class="breadcrumb"><a href="index.html">الاستقبال</a> ← السيرة الذاتيّة</div>
      <h1>السيرة الذاتيّة</h1>
      <p class="lead">المسار المهني والأكاديمي للتلميذة المتفقّدة نجلاء الساحلي</p>
    `;
        ...
        <div>
          ...
          <div class="cv-section">
            ...
            <div class="cv-timeline">
              ${(cv.parcoursAcademique || []).map((it) => `
                <div class="cv-item">
                  <div class="when">${it.periode || '—'}</div>
                  <div class="what">
                    <h4>${it.titre || ''}</h4>
                    <div class="where">${it.lieu || ''}</div>
                    <div class="desc">${it.details || ''}</div>
                  </div>
                </div>
              `).join('')}
            </div>
          ...
          <div class="cv-section">
            ...
            <div class="cv-timeline">
              ${(cv.experiences || []).map((it) => `
                <div class="cv-item">
                  <div class="when">${it.periode || '—'}</div>
                  <div class="what">
                    <h4>${it.poste || ''}</h4>
                    <div class="where">${it.etablissement || ''}</div>
          ...
          <div class="cv-section">
            ...
            <div class="cv-timeline">
              ${(cv.formations || []).map((it) => `
                <div class="cv-item">
                  <div class="when">${it.date || '—'}</div>
                  <div class="what">
                    <h4>${it.titre || ''}</h4>
                    <div class="where">${it.organisme || ''}</div>
                  </div>
                </div>
              `).join('')}
            </div>
        ...
        <div>
          ...
          <div class="cv-section">
            ...
            <div>
              ${(cv.langues || []).map((l) => `
                <div class="cv-lang">
                  <span><strong>${l.langue}</strong></span>
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    /* الصفحات التي تحتاج محتوى مفصّل (الوحدات والمجالات) */
    const needsContent = ['unit'];
    
    if (needsContent.includes(page) && window.CONTENT_MANIFEST) {
      /* انتظر حتى يكتمل تحميل كل ملفّات المحتوى */
      if (window.CONTENT_LOADED === window.CONTENT_MANIFEST.length) {
        runPage(page);
      } else {
        document.addEventListener('content-ready', () => runPage(page));
        /* احتياط: لو لم يُطلق الحدث خلال 5 ثوان، نشغّل بأيّ حال */
        setTimeout(() => runPage(page), 5000);
      }
    } else {
      runPage(page);
    }
  ...
  function buildInlineToc() {
    ...
    /* تفعيل lightbox لصور الوثيقة */
    enableFiguresLightbox(body);
  ...
  /* تفعيل Lightbox لكل صور الوثيقة - نقرة تفتح الصورة بحجم كامل */
  function enableFiguresLightbox(container) {
    if (!container) return;
    
    /* أنشئ lightbox واحد إن لم يوجد */
    let lb = document.querySelector('.figure-lightbox');
    if (!lb) {
      lb = document.createElement('div');
      lb.className = 'figure-lightbox';
      lb.innerHTML = '<button class="close-btn" aria-label="إغلاق">✕</button><img alt="">';
    ...
    /* اربط كل صور الوثيقة بالـ lightbox */
    const figures = container.querySelectorAll('.doc-figure img');
    figures.forEach(img => {
